...
- (void)updateIndicators {
	...
	if (mode == MBProgressHUDModeIndeterminate) {
		...
		if (mode == MBProgressHUDModeAnnularDeterminate) {
			[(MBRoundProgressView1 *)indicator setAnnular1:YES];
		}
	} 
...
- (void)drawRect:(CGRect)rect {
	...
	if (self.dimBackground) {
		...
		CGFloat gradColors[8] = {1,1,1,0,1,1,1,0.75f};
...
//    float height=allRect.size.height;
//    float width=allRect.size.width;
//
//    float x=allRect.origin.x;
//    float y=allRect.origin.y;
//
//    
//    
//    if (height==627) {
//        height=603;
//        allRect=CGRectMake(x, y, width, height);
//    }
//    NSLog(@"%f",height);
//    
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		_progress1 = 0.f;
		_annular1 = NO;
		_progressTintColor1 = [[UIColor alloc] initWithWhite:1.f alpha:1.f];
		_backgroundTintColor1 = [[UIColor alloc] initWithWhite:1.f alpha:.1f];
		[self registerForKVO];
	}
	return self;
}
...
- (void)drawRect:(CGRect)rect {
	...
	if (_annular1) {
		...
		[_backgroundTintColor1 set];
		[processBackgroundPath stroke];
		// Draw progress
		UIBezierPath *processPath = [UIBezierPath bezierPath];
		processPath.lineCapStyle = isPreiOS7 ? kCGLineCapRound : kCGLineCapSquare;
		processPath.lineWidth = lineWidth;
		endAngle = (self.progress1 * 2 * (float)M_PI) + startAngle;
		[processPath addArcWithCenter:center radius:radius startAngle:startAngle endAngle:endAngle clockwise:YES];
		[_progressTintColor1 set];
		[processPath stroke];
	} else {
		// Draw background
		[_progressTintColor1 setStroke];
		[_backgroundTintColor1 setFill];
		...
		CGFloat startAngle = - ((float)M_PI / 2); // 90 degrees
		CGFloat endAngle = (self.progress1 * 2 * (float)M_PI) + startAngle;
